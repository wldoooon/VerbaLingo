...
export default function SearchBar({ query, onQueryChange }: { query: string; onQueryChange: (q: string) => void }) {
  ...
  return (
    <div className="w-full max-w-lg">
      ...
      {isLoading && <p className="mt-2 text-center text-sm text-muted-foreground">Searching for clips...</p>}
      {error && <p className="mt-2 text-center text-sm text-destructive">{error.message}</p>}
      {data && (
        <p className="mt-2 text-center text-sm text-muted-foreground">
          Found {data.total} clips containing "{query}"
        </p>
      )}
    </div>
  )
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          role="combobox"
          aria-expanded={open}
          className="mr-2 flex shrink-0 items-center rounded-md bg-secondary px-2 py-0.5 text-xs font-semibold text-secondary-foreground hover:bg-secondary/80"
        >
          {value}
