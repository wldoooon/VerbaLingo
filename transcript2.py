...
def _direct_json3_from_info(info: dict, headers: dict):
    for cc_kind in ('automatic_captions', 'subtitles'):
        ...
        for lang_key in (k for k in cc.keys() if k and k.startswith('en')):
...
def download_transcript(video_id):
    ...
    try:
        with yt_dlp.YoutubeDL(json3_opts) as ydl:
            ...
            if not subs_info or 'en' not in subs_info:
        ...
        json3_path = f'{video_id}.en.json3'
...
if __name__ == "__main__":
    input_jsonl = 'BoxofficeMoviesScenes_ENG.jsonl'
    output_jsonl = 'BoxofficeMoviesScenes_ENG_Dataset.jsonl'
